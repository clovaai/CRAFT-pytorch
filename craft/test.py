...
def test_net(net, image, text_threshold, link_threshold, low_text, cuda, refine_net=None):
    ...
    ret_score_text = imgproc.cvt_2_heatmap_img(render_img)
...
if __name__ == '__main__':
    ...
    if args.cuda:
        ...
        net.load_state_dict(copy_state_dict(torch.load(
    ...
    if args.refine:
        ...
        if args.cuda:
            refine_net.load_state_dict(
                copy_state_dict(torch.load(args.refiner_model)))
            refine_net = refine_net.cuda()
            refine_net = torch.nn.DataParallel(refine_net)
        else:
            refine_net.load_state_dict(copy_state_dict(
